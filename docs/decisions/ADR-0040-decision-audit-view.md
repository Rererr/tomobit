...
## 追記（2026-07-27）: サブタスクは `decided` を流さなくなった

Decision 1 の箇条書きの1行が事実でなくなった:

> split（ADR-0023/0028）の下では各サブタスクが自分の sid 付きで独立に `decided` を流す

[ADR-0054](ADR-0054-a-child-is-the-breakdown.md) Decision 1 が、分割の子から
決定そのものを外したためである。**1タスク = 1決定**なので、`decided` は
タスクを開くときの1回だけになった。子は親が選ばれた相手で走る。

`sid` の相関規則（Decision 1）は不変で、むしろ簡単になる — 相関先が親セッション
1つしか無い。読み手側の変更も要らない: GUI は「直前の `decided` を、続く
`task.started` の sid と突き合わせる」形なので、来なければ子にバッジが出ないだけで
壊れない（実装で確認済み）。

**出なくなったことに意味がある。** 子ごとのバッジは「この子にはこの Provider を
選んだ」と読ませるが、実際には親の知覚トークンで同じくじを K 回引いていただけで、
見せていたのは判断ではなく抽選の結果だった。ADR-0040 の目的は「なぜこの Provider か」
を開示することなので、答えられない問いにバッジを出す方が本ADRの趣旨に反する。

duel（ADR-0026）の子は変わらず自分の `decided` を持たない — あちらは gap が
2側を指名するので、そもそも決定エンジンを通らない。
