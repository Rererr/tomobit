...
## 追記（2026-08-01）: 語彙表が2つ取りこぼしていた — `permission` と `reaction`

Decision 1 の語彙表は「増やすのは互換」と自分で書いた**生きた契約**なので、
型が増えたら追記されるべきだった。実際には `sub` / `sub_total` の追記（上）の後、
**2つの型が表に載らないまま実装されていた**。ADR-0057 の起票時に発見したので、
まとめてここへ載せる（ドリフトを見つけた側が閉じる）。

```json
{"type":"permission","text":"…","tools":[…],"await":true}   道具の許可を求める問い（ADR-0053 Decision 5）
{"type":"reaction","n":3,"word":"up"}                        置かれた反応の確認（ADR-0057 Decision 3）
{"type":"init","v":1,"reactions":[{"word":"up","label":"文句なし"}, …]}  反応の語彙（同）
```

- `permission` は**文面から種類を当てさせない**ために型を足した前例である
  （ADR-0053 Decision 5）。`note`+`await` に混ぜると、消費者は日本語の文言で
  分岐することになり、本体が言い回しを変えた日に黙って壊れる
- `reaction` は台帳への記帳（`user.reaction`）の**確認**で、消費者が
  「押した通りに描かない」ためにある（GUI ADR-0010 Decision 3 と同じ姿勢）
- `init` にキーが1つ増えた。**型は増えていないので `v` は上げない** —
  知らない読み手は今までどおり `v` だけを見る（消費者規律のとおり）。
  受け取らなかった消費者が反応の口を出さないのは、劣化として正しい

### なぜ表が腐ったのか（Why not: 「気をつける」で終わらせない）

型を足す場所（`chat.go` の emit）と、契約を書いた場所（このADR）が離れている。
`make docs-check` はリンクと改版マークを見るが、**語彙表と実装の一致は見ていない**。
ADR-0053 は自分のDecisionの中に新しい型を書いており、それで済んだと読める形だった。

いまは追記で塞いだが、構造的な再発防止は入れていない（語彙表を実装から生成する、
あるいは表と `chat.go` の emit を突き合わせる検査を足す、が候補）。**塞いでいない
ことを書いておく方が、塞いだつもりで放置するより正直である。**
