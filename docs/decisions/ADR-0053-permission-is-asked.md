...
- **ディスクに書かない**。「毎回許可するのが面倒だから覚えておく」は、
  ...
  次に人が見ていない時にも効く
- **分割の子はこの寿命の内側にいる**（2026-08-08 追記。実装漏れの修正であって
  決定の変更ではない）。子は独立セッション（`sid`）だが、[ADR-0054](ADR-0054-a-child-is-the-breakdown.md) が決めたとおり
  **タスク1つの内訳**であって別のタスクではない。人が答えた「はい、Edit を」は
  このタスクについての答えなので、内訳に分かれた瞬間に消えるものではない。
  漏れていた間、`subtaskWiring` は `PermissionMode` を継ぎながら `AllowedTools`
  だけを落としており、子は許可済みの道具を訊き直すか、誰も端末にいなければ
  Decision 5 の「答えが来ないので拒否」で**人が既に許した作業を落としていた**
