...
> 届くようにした。
>
> **さらに改版（同日の実測）**: その移し先も成立しなかった。distinguishing tokenの
> ln BFは減衰につれ`ThetaMerge = 0`へ**上から** d² のオーダーで漸近するだけで、
> 判定が真になるのはfloat64の桁が落ちて厳密に0.0になる約60半減期（≒15年）である
> （ADR-0037「実測による訂正」に計測値）。**Decision 3の名誉回復は継承事前の下で
> 未解決だった。** 解決は[ADR-0038](ADR-0038-gate-under-inherited-priors.md)（Accepted）—
> ゲートの基準線を定数qからその Connection 自身の事前のq分位点（`min(q, PriorQuantile(q))`）へ
> 一般化し、**証拠が減衰しきった＝何も分かっていない状態はどの家系でも土俵に立つ**ようにした。
> 一様事前の根では今日と同一の判定に落ちる。これで Decision 3 の約束は継承事前の下でも成立する。
> ADR-0037が入れたmerge判定の到達性修復自体も有効で、新しい反証の証拠でln BFが負に振れた子は
> 触れられていなくても畳まれる。
...
> **改版（[ADR-0036](ADR-0036-task-perception-wiring.md) Decision 2）**:
> 判断が読むトークンにタスク知覚（LLM）の出力が入ったため、
> **「同じ台帳＋同じseed」だけでは判断を再現できない** — 同じ台帳・同じseedでも、
> 抽出器が別のトークンを返せば別のConnectionが読まれる。
> 再現の単位は**台帳＋seed＋tokens**になった。
> だから `tomo.decided` / `plan.selected` の payload に、実際に判断が読んだ
> トークン列を記帳する（劣化したときはその理由も）。
> 決定則そのものは純関数のままで、入力が1つ増えただけである —
> 記帳されない入力があれば監査可能性が失われるが、記帳するなら失われない。
