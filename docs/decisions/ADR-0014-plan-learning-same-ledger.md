...
- 実装追記（2026-07-16、`internal/plan` + engine/decide/curiosity統合）:
  ...
  - 変異優先度: 全候補が白紙で揺らぎ同値のため、VoIは分離できない —
    ...
    提案予算は24hに1つ（質問予算の型 = ADR-0007の`tomo.asked`チェックと同じ、
    **capability横断でハーネス全体に1つ**）、メニュー空き（K=5未満）時のみ。
    K（Decision 5）はcapabilityごとの生存上限だが、提案予算はそれとは別の
    ノブで、質問予算と同じくスコープを切らない全体ゲート
  - **「新規」の判定はメニューでなく生成履歴全体**: `Propose`が「メニューに
    無い最初の合法変異」を候補にすると、引退した変種（メニューから落ちた
    だけで名前は生きている）が毎回「メニューに無い最初の変異」として復活し、
    その先の変異が永久に試されない（列挙順の先頭で詰まる）。既存/退役を
    問わず`plan.generated`された全名前を恒久的に除外対象へ入れることで
    列挙が前進する。この恒久除外はRevivedを殺さない —
    `--plan <steps>`の明示指定は`plan.Resolve`がメニュー非依存で受理し、
    `plan.selected`→経験→`Connection`更新（`kind=plan`）の経路は
    `Propose`を経由しないため、引退株へ証拠が再流入すればdormancy判定
    （`nowMs-LastUpdate <= 2*halfLife`）は自然に外れる。専用の「復活」
    状態は追加しない
