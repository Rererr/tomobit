...
- 実装追記（2026-07-16、`face.StageFrom`）: 前提部品（台帳=ADR-0002実装済み、
  TSサンプラー=ADR-0012 `internal/decide`）が揃ったため量ゲートを置換。
  初期ノブ: θ_cal=0.15 nats、θ_sharp=0.2（M=128本、seed固定 —
  ステージがViewであり続けるため同じDBは常に同じ顔を見せる）、
  頻繁な島の下限頻度=2.5（CuriosityのFMinと同値、ADR-0016と共有）。
  鋭さは「頻繁な島が1つ以上あり、その全てで揺らぎ≤θ_sharp」。
  ダッグフードでの実測に応じて調整する

## 較正観測の追記（2026-07-24、tools/dogfood の合成長期dogfood）

「ダッグフードでの実測に応じて調整する」の最初の材料:

- **stage が1バッチで 1→5 に跳躍した**。13セッション+選好回答2件で最上位
  「あいぼう」到達（S3較正・S4シャープネス・S5選好evidence≥1.0 は、一方の
  Providerが一貫して勝つ偏ったデータでは全て即通過する）。声は
  「少し育った気がする」のまま4段跳びで、表現とのズレもある。合成データの
  偏り故か、ノブが緩いのかは実運用の台帳で要判定
- **auto の本番タスクの 25–37% が human に流れた**。blank な選好ペアが
  Beta(1,1) のコイントスになるため（ADR-0018 の設計どおりだが、`--size`
  未指定 n=1 の既定では体感が大きい）。探索の按分をノブにするかは
  ADR-0012/0018 の管轄

数値の出所と再現: tools/dogfood（obs 一式はハーネスが再生成する）。
