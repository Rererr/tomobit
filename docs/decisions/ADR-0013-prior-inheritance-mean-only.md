...
> **改版注記（2026-07-24, [ADR-0042](ADR-0042-split-starvation-and-lexical-shadowing.md) 対案2）**:
> 「読むのは一つ」は選択（TS・順位付け）については不変のまま、
> **悲観ゲートだけは同粒度一致の全員が拒否できる**ようになった —
> 「選ぶのは一つ、拒否は全員ができる」。同粒度タイの片側が連敗している
> とき、辞書順の遮蔽で危険が判断に届かない実測（11連敗Providerが最頻選択）
> への対処。監査の一意性は保たれる（通過時=読んだ行、拒否時=拒否した行）
