...
## 追記（2026-07-26）: ADR-0050 — duel の子は各自の作業場を持つ

本ADRは実験の同型性を丁寧に設計したが、**2本が同じ作業ツリーを共有していること**は
論点に上がっていなかった。同一プロンプトを2 Provider が同時に走らせる形は、
**互いの成果が互いに見える**という点で、あらゆる並走のなかで比較実験として最も
汚染が痛い。片方が `checkout` すればもう片方の前提が崩れ、両方が同じファイルを
書けば片方の成果は静かに消える。

[ADR-0050](ADR-0050-workspace-isolation-protocol.md) はこれを塞ぐ。隔離の単位は
セッション木に1つ（分割サブタスクは親の作業場を継ぐ — 群間依存を壊さないため）だが、
**duel の子だけは各自1つ持つ**。2本は互いに依存しないので、分けても壊れる前提が無い。

Decision 1〜5 に変更はない。実験の同型性が、初めて作業場の水準でも成立する。

なお ADR-0050 は**群内並走（ADR-0028 Decision 2）の共有は塞がない**（塞ぐと群間
依存が壊れ、独立宣言を構造で二重に縛ることになるため）。塞ぐのは「誰も独立を宣言して
いない並走」— duel・複数の起動・GUI の四分割 — だけである。

---

## 追記（2026-07-27）: duel の子は、唯一の「独立した発注」として残る

[ADR-0054](ADR-0054-a-child-is-the-breakdown.md) が、親を持つセッションは既定で
経験にならないと決めた（分割はタスク1つの内訳なので、経験は親の1件）。
**duel はその唯一の例外である。**

理由は本ADRが最初から書いていること — duel は**2者を比べるために、わざと別々に
指名している**。子が独立した発注であることが実験の前提であって、そこを内訳に
まとめたら実験そのものが消える。

例外は opt-in の形で書かれている: duel の子は、親が記帳した `task.duel` によって
自分を名指しで外す。つまり**将来どんな親子関係が生まれても既定は「内訳」**で、
独立した発注として扱いたければ明示が要る。ADR-0054 の原則がそちら側だからである。

実装上の変更が1つある。`openSubtask` は Provider を解決しなくなった（ADR-0054
Decision 1）が、**duel は元から影響を受けない** — `pair` は常に2つの具体名なので、
あの関数の `auto` 分岐は duel からは構造的に到達不能だった。いまは呼び出し側が
`providers[p]` を直接引く形になり、そのことがコードから読めるようになっただけである。

---

## 追記（2026-07-30）: 同型性は、アームの道具箱の中までは届かない

本ADRは実験の同型性を丁寧に設計した。**その設計に、当時気づいていなかった穴がある。**

2026-07-30 の実測: chat のタスクで claude-code が Bash 経由で codex CLI を起動した
（所有者が「codex でも並列で」と依頼した回。詳細は
[ADR-0060](ADR-0060-the-executor-can-be-named.md) Context）。Provider の道具箱には
**別の Provider の CLI が入っている**。これは duel でも起こりうる。

つまり duel の比較は、最悪の場合「codex 単体 vs claude-code ＋ codex」になる。
本ADRが同型性として揃えたのは、プロンプト・作業場（ADR-0050）・記帳の形であって、
**アームが内部で誰を呼ぶか**ではない。

対処は**しない。理由を書いて残す。**

- **塞げない。** 道具箱から CLI を1つ抜いても、`curl` も MCP も Provider 自身の
  サブエージェントも残る。塞げるのは「たまたま目に見えた形」だけで、
  **塞いだつもりになるほうが危ない**
- **判定は人がする。** duel の一次審判はユーザーである（Decision 3）。そして
  ツール使用の `detail` は view に出ているので、**入れ子が起きたことは画面から読める**。
  判定者が見て判断できる材料は既にある
- **記録には残らない。** `StripViewOnly` が記帳前に `detail` を落とすので、
  `user.preference` の経験には「入れ子があった」ことが残らない。
  これは既知の限界として本追記が引き受ける

**duel の2本目は入場のプールから取る**（[ADR-0059](ADR-0059-admission-is-counted-by-the-machine.md)
Decision 1）。取れなければ待つ。duel は好奇心であって仕事ではないので、待たせてよい —
VISION の Curiosity with Responsibility が、仕事の並走との扱いの差に根拠を与える。
