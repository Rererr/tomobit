...
```
  ...
  実効証拠の和が 2×n_min あるため、f_min ≤ 2×n_min の間、頻繁ゲートは
  互角ゲートに含意され単独の閉塞要因にならない。
  頻繁ゲートを実質化するなら f_min > 2×n_min に上げる

  実装時の計測メモ2（2026-07-15・有機E2Eで実測）: 離散経験×連続減衰の
  相互作用により、n回の新鮮な経験の実効証拠は観測直後からnをわずかに
  割り込む（3+3回のdoで証拠2.9999 — 「≥3.0」は永遠に開かない）。
  逆向きにe_max=1.0は、1回の回答（証拠1.0）が数秒後に1.0を割って
  「知っている好みをまた聞く」を許してしまう。実装ノブは意図を保存して
  n_min/f_min=2.5（3回で開き、約24日の減衰まで開いたまま）、
  e_max=0.5（1回の回答が約1半減期=90日拘束し、その後は鮮度切れとして
  再質問してよい — 好みの再確認はむしろ望ましい）とする。
  ノブ表の3.0/1.0は「回数」の意図表記として残し、実効値はコードの
  const（internal/curiosity）を正とする
