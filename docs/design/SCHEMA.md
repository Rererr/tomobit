...
R4. eventsのtype初期カタログ（14種で確定）
    ...
    **抽出プロンプトが見るのは「起きたこと」だけ**（ADR-0036 Decision 2d）:
    ...
    - task.split（ADR-0023、ADR-0028で拡張、ADR-0056で縮小）: 受理した分割提案の
      ...
      **実行者を指名された子だけが named_provider を持つ**（ADR-0060 D4）:
      task.started payload に宣言された Provider 名を足す（既定＝親継承の子は
      キー自体が無い）。指名された実行の成否は抽選で選ばれた実行と同じ母集団
      ではない（人が名指しするのは「その相手向きだと思ったとき」）ので、
      混ぜたまま事後分布に入れると auto の学習が歪む。**分離規則は決めない —
      分離できないデータにする事故だけを避ける**（ADR-0044 の「観測と推定を
      分ける」と同じ作法）。記録するのは**宣言された名**であって解決結果では
      ない: 未知名・human・kill switch で親継承に落ちた子はキーを持たず、
      起きなかった配線を主張しない
