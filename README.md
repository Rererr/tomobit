...
- [ADR-0042](docs/decisions/ADR-0042-split-starvation-and-lexical-shadowing.md) — Splitの飢餓と辞書順の遮蔽・問題提起（均衡混合でexcess surprisalが発火せず、同粒度tie-breakの辞書順でlang=系Connectionが系統的に読まれない — 11連敗Providerが最頻選択される実測 / 対策は判断数学に触れるため対案列挙のみ、裁定待ち。Proposed・未実装）
